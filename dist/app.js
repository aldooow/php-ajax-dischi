  ...
  */
  $.ajax({
    ...
    success: function success(data) {
      ...
      var template = Handlebars.compile(source); // Variante di tutto il contenuto del URL (Array di OBJECTS).

      var cds = data; // Ciclo FOR con Lunghezza della Variante che contiene tutta la risposta URL.

      for (var i = 0; i < cds.length; i++) {
        // Variante di singolo OBJECT che è dentro l'Array.
        var cd = cds[i]; // TEMPLATE da Appendere.

        var html = template(cd); // Appendere TEMPLATE HTML nel Elemento desiderato.
  ...
  */
  ...
  $.ajax({
    ...
    success: function success(data) {
      ...
      var cds = data;
      var authorAll = []; // Ciclo FOR con Lunghezza della Variante che contiene tutta la risposta URL.

      for (var i = 0; i < cds.length; i++) {
        // Variante di singolo OBJECT che è dentro l'Array.
        var cd = data[i];
        var author = cd['author']; // Se AUTHOR non è ancora incluso, lo stampo nel SELECT, Altrementi non lo aggiungo di nuovo.

        if (authorAll.includes(author) == false) {
          authorAll.push(author); // TEMPLATE da Appendere.
  ...
  $(document).change(function () {
    ...
    var genreValueSelect = $("#js_select").val(); // Per Ogni CD esegue questa funzione.

    $(".cd").each(function () {
      // Creare una Variante Per il GENRE di THIS CD.
      var authorCD = $(this).find(".author").text(); // Se il VALUE del SELECT è uguale ad "All", Mostrare Tutti CD.
...
__webpack_require__(/*! /Users/aldo/Library/Mobile Documents/com~apple~CloudDocs/BOOLEAN/repo/mamp-htdocs/php-ajax-dischi/src/app.js */"./src/app.js");
module.exports = __webpack_require__(/*! /Users/aldo/Library/Mobile Documents/com~apple~CloudDocs/BOOLEAN/repo/mamp-htdocs/php-ajax-dischi/src/app.scss */"./src/app.scss");
